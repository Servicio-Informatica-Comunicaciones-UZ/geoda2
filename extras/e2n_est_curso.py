...
#
# Este script mirando los usuarios estudiantes (que tienen su
# usuario de correo como `username`, y su NIP como `idnumber`) del curso indicado.
# Averigua cuál es el usuarioNip de cada estudiante.
# Matricula al usuarioNip correspondiente como estudiante del curso.
#
# Dependencias:
...
parser = argparse.ArgumentParser(description="Matricula estudiantes de un curso con su usuarioNip")
parser.add_argument("course_id", help="ID del curso en Moodle", type=int)
...
def request_url(verb, api_url, wsfunction, token, data=None):
    ...
    if verb == 'POST':
        resp = requests.post(
            f'{api_url}?wstoken={token}&wsfunction={wsfunction}&moodlewsrestformat=json', data=data
        )
    elif verb == 'GET':
        resp = requests.get(
...
def main():
    ...
    try:
        with connection.cursor() as cursor:
            # Estudiantes de este curso que no tienen su NIP en el `idnumber`.
            cursor.execute(
                f'''
            ...
            # Asignaciones de alumnos de este curso académico que tienen su NIP en el `idnumber`.
            cursor.execute(
                f'''
            ...
            for asignacion in asignaciones_usuarios_correo:
                ...
                # Matriculamos como estudiante del curso al usuarioNip
                payload = {
