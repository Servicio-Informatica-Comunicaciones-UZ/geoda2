...
def dictfetchone(cursor):
    ...
    raise UsuarioNoEncontrado('Usuario desconocido. No se ha encontrado en Gestión de Identidades.')
...
def get_identidad(strategy, response, user, *args, **kwargs):
    ...
    with connections['identidades'].cursor() as cursor:
        cursor.execute("SELECT * FROM GESTIDEN.GI_V_IDENTIDAD WHERE NIP = %s AND INDBAJA = 'N'", [user.username])
        identidad = dictfetchone(cursor)
        cursor.execute('SELECT * FROM GESTIDEN.GI_V_IDENTIDAD_PERFIL WHERE NIP = %s', [user.username])
        perfiles = dictfetchall(cursor)
        cursor.execute('SELECT * FROM GESTIDEN.GI_V_IDENTIDAD_VINCULACION WHERE NIP = %s', [user.username])
    ...
    correo_personal = identidad.get('CORREO_PERSONAL') if is_email_valid(identidad.get('CORREO_PERSONAL')) else None
    correo_principal = identidad.get('CORREO_PRINCIPAL') if is_email_valid(identidad.get('CORREO_PRINCIPAL')) else None
    ...
    perfil_centro_id_nks = {
        perfil.get('COD_CENTRO') for perfil in perfiles if perfil.get('COD_CENTRO') not in (0, None)
    }
    ...
    perfil_departamento_id_nks = {
        perfil.get('COD_DEPARTAMENTO') for perfil in perfiles if perfil.get('COD_DEPARTAMENTO') not in (0, None)
    }
    ...
    departamento_id_nks = list(perfil_departamento_id_nks.union(vinculacion_departamento_id_nks))
    ...
    cod_vinculaciones = {vinculacion.get('COD_VINCULACION') for vinculacion in vinculaciones}
